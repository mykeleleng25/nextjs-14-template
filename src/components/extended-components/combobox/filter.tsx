...
import type { Product } from '@/types/ui/combobox'
import { Search } from '@/components/extended-components/combobox/search'
import { useQuery } from '@tanstack/react-query'

const POPOVER_WIDTH = 'w-[250px]'

export function ComboboxDemo({ form }: any) {
  ...
  const handleSetActive = React.useCallback((product: Product) => {
    setSelected(product)
    form.setValue('phone', product.id.toString())
